...
from dellus.core.initialize import initialize_test
from dellus.app.link import shorten, unshorten
from dellus.config import config
from dellus.exception.auth import URLAuthFailed
...
class URLShortenUnshortenTestCases(TestCase):
    ...
    def setUp(self):
        self.long_url = 'https://google.com/myimage/log'

    @classmethod
    def setUpClass(cls):
        currdir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        config_path = currdir + '/config/dellus.cfg'
